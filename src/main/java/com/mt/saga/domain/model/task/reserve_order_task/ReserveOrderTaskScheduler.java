...
public class ReserveOrderTaskScheduler {
    ...
    public void rollbackTask() {
        ...
        List<ReserveOrderTask> tasks = DomainRegistry.getReserveOrderTaskRepository().findRollbackTasks(from);
        if (!tasks.isEmpty()) {
                    ...
                    new TransactionTemplate(transactionManager)
                            .execute(new TransactionCallbackWithoutResult() {
                                ...
                                protected void doInTransactionWithoutResult(TransactionStatus status) {
                                    ...
                                    Optional<ReserveOrderTask> byIdOptLock = DomainRegistry.getReserveOrderTaskRepository().findByIdLocked(task.getId());
    ...
    private void cancelReserveTask(ReserveOrderTask bizTx) {
        ...
        CompletableFuture<Void> updateOrderFuture = CompletableFuture.runAsync(() ->
                DomainRegistry.getOrderService().cancelReserveOrder(command, bizTx.getCancelTaskId(), command.getTxId()), customExecutor
        );
        ...
        DomainRegistry.getReserveOrderTaskRepository().add(bizTx);
    }
